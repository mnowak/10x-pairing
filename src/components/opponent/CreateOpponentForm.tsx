...
export default function CreateOpponentForm({ serverError }: Props) {
  ...
  function validate() {
    ...
    if (!name.trim()) {
      next.name = "Opponent name is required";
    }
  ...
  return (
    <form method="POST" action="/api/opponents" className="space-y-4" onSubmit={handleSubmit} noValidate>
      <FormField
        id="name"
        label="Opponent name"
        value={name}
      ...
      <SubmitButton pendingText="Adding opponent..." icon={<Swords className="size-4" />}>
        Add opponent
      </SubmitButton>
    </form>
  );
