...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "matrix input shape"
      decision: "REVISED 2026-09-06 (during F-01 implementation): pairing-matrix cells store a raw integer point estimate (0-20 — how many of the 20 match points our army is expected to score), not a color band. Color-band display is derived in the application layer from the stored number. Originally decided as stored color bands in this same entry (see FR-004's Socratic note for the reversal rationale: the numeric value is strictly more precise and the band is a pure function of it, so nothing was lost by storing the number instead). REVISED AGAIN 2026-09-07 (during S-02 implementation): the same revision folded \"unpredictable\" (formerly the purple band) into \"leave the pair unestimated\" — that turned out to conflate \"not yet assessed\" with \"deliberately called unpredictable,\" which captains found costly to lose. Purple is reinstated as an explicit, separately-stored marker (nullable score + is_purple boolean); a blank cell now means only \"not yet assessed.\""
...
- FR-004: Captain can enter a pairing-matrix estimate for every one of our armies vs. every one of the opponent's armies: either a point estimate (integer, 0–20 — how many of the 20 match points our army is expected to score) or an explicit "purple" marker for a matchup judged too unpredictable to call. A pair left unestimated means only "not yet assessed." Priority: must-have
  > Socratic: Counter-argument considered (revisited 2026-09-06, during F-01 implementation): color bands were originally chosen because captains think in bands during live prep (per worksheet) — but the raw 0–20 point estimate is strictly more precise, and a color-band display is a pure function of the number, so nothing is lost by storing the number and deriving the band in the application layer. Resolution: FR reverted to numeric input; the UI still shows color bands, derived from the stored integer, not stored directly. "Unpredictable" was folded into "leave unestimated" at this point, then reinstated as its own explicit purple marker on 2026-09-07 (see foundation entry above and the current prd.md FR-004 note) once that fold-in proved to conflate "not yet assessed" with "deliberately called unpredictable."
